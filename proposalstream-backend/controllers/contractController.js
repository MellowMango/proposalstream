...
export const createContract = async (req, res) => {
  try {
    const { proposalId, mergedContractPdf } = req.body; // Extract mergedContractPdf from request body
    ...
    if (!mergedContractPdf) {
      return res.status(400).json({ error: 'mergedContractPdf is required' });
    }
    ...
    const newContract = new Contract({
      ...
      mergedContractPdf, // Set the mergedContractPdf field
